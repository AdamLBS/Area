...
export default class YoutubeCheckLike extends BaseTask {
  ...
  private async fetchYoutubeData(oauth: string): Promise<YoutubeData> {
    try {
      const response = await axios.get<YoutubeData>(`https://www.googleapis.com/youtube/v3/videos?myRating=like&part=snippet`, {
        headers: {
    ...
    video: Item) => {
      for (const field of fields) {
        ...
        if ((field.value as string).includes('$videoUrl')) {
          field.value = field.value.replace('$videoUrl', `https://www.youtube.com/watch?v=${video.id}`)
        }
      }
    }
